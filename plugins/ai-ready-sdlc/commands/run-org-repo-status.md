...
- initialize the local layout with `python3 scripts/init-org-status-repo.py --repo-path <collection-repo> --org <github-org>` when needed,
...
- publish each repo's own evidence bundle with `python3 scripts/publish-adlc-snapshot.py --repo-path <target-repo> --org jasper-sw --collection-owner jasper-sw --collection-repo adlc-org-status --payload-output /tmp/adlc-payload.json`,
